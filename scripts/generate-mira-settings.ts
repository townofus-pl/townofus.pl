...
/**
 * The TOU-Mira release these settings describe, read from the submodule's own tag.
 *
 * Emitted here rather than typed into a page so the two can never disagree about which TOU-Mira
 * the site is talking about: whoever bumps the submodule re-runs this and the version follows.
 * `/do_pobrania` links the matching upstream release. See #316.
 */
function writeMiraVersion(modPath: string): void {
    const submodule = path.join(modPath, 'TOU-Mira');
    let tag: string;
    try {
        tag = execFileSync('git', ['-C', submodule, 'describe', '--tags'], { encoding: 'utf8' }).trim();
    } catch {
        throw new Error(
            `Could not read a tag from ${submodule}. The submodule must be checked out at a ` +
                `released tag, because that is the build players are told to install.`,
        );
    }
    ...
    if (!/^\d+\.\d+(\.\d+)?$/.test(version)) {
        throw new Error(
            `TOU-Mira is at "${tag}", which is not a plain release tag. Players are pointed at an ` +
                `upstream release, so the submodule has to sit on one.`,
        );
    }

    writeFileSync(
        'src/roles/_generated/miraVersion.ts',
        [
            ...
            '',
            `export const MIRA_VERSION = '${version}';`,
            '',
    ...
    console.log(`mira version written: ${version}`);
}
