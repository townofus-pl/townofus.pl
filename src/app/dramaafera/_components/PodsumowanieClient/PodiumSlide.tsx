...
import { videotext, PODIUM_STANDARD_STEPS, PODIUM_TIE_STEPS, PODIUM_SWAP_DURATION } from './constants';
...
interface PodiumSlideProps {
    ...
    date: string;
    currentStep: number;
    podiumSwapSecondRevealed: boolean;
    ...
    return (
        <>
            ...
            {hasThirdPlaceTie ? (
                // Przy remisie: krok 2=historia 1. z remisu, 3=historia 2. z remisu, 6=historia 2., 9=historia 1.
                <>
                    {currentStep === 2 && sortedStats[2] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[2].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
                    {currentStep === 3 && sortedStats[3] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[3].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
                    {currentStep === 6 && sortedStats[1] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[1].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
                    {currentStep === 9 && sortedStats[0] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[0].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
                </>
            ) : (
                // Normalnie: krok 2=historia 3., 5=historia fake 2. czyli 1., 8=historia prawdziwego 2.
                <>
                    {currentStep === 2 && sortedStats[2] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[2].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
                    {currentStep === PODIUM_STANDARD_STEPS.fakeSecondHistory && sortedStats[0] && topPlayerGames.length > 0 && <PlayerHistory nickname={sortedStats[0].nickname} isFullscreen={isFullscreen} topPlayerGames={topPlayerGames} playerRankingChanges={playerRankingChanges} date={date} />}
