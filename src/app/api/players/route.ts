  ...
  request: {
    query: z.object({
      ...
      search: z.string().optional().openapi({
        description: 'Search players by name (case-insensitive)',
        example: 'malk'
      }),
...
openApiRegistry.registerPath({
  method: 'post',
  path: '/api/players',
  description: 'Create a new player with case-insensitive duplicate checking',
  ...
  responses: {
    ...
    409: {
      description: 'Player already exists (case-insensitive match)',
      content: {
        'application/json': {
          schema: ErrorResponseSchema
        }
