...
import { generateRoleRankingStats, type RoleRankingStats } from '../_services/gameDataService';
...
export default function RolePage() {
    ...
    const [roleStats, setRoleStats] = useState<RoleRankingStats[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    // Ładowanie danych statystyk ról
    useEffect(() => {
        async function loadRoleStats() {
            try {
                setIsLoading(true);
                const stats = await generateRoleRankingStats();
                setRoleStats(stats);
            } catch (error) {
                console.error('Błąd ładowania statystyk ról:', error);
            } finally {
                setIsLoading(false);
            }
        }
                                                ...
                                                >
                                                    ...
                                                    <td className="py-2 px-2 w-64">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <Link
                                                                    href={`/dramaafera-new/role/${convertRoleToUrlSlug(role.name)}`}
