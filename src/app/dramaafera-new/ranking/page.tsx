...
import { generatePlayerRankingStats, type PlayerRankingStats } from "../_services/gameDataService";
...
export default function RankingPage() {
    const [playerStats, setPlayerStats] = useState<PlayerRankingStats[]>([]);
    const [sortBy, setSortBy] = useState<keyof PlayerRankingStats>("rankingPoints");
    ...
    useEffect(() => {
        (async () => {
            try {
                setIsLoading(true);
                const stats = await generatePlayerRankingStats();
                                ...
                                {sortedStats.map((player, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="py-2 px-2">
                                                <Link href={`/dramaafera-new/user/${convertNickToUrlSlug(player.name)}`}>
